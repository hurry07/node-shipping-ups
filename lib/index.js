...
function UPS(args) {
  const $scope = this,
    ...
    canada_origin_services = {
      ...
      '07': 'UPS Express to USA', // Should be 01 when doing Ship calls
    },
    ...
    other_non_us_origin_services = { '07': 'UPS Express', },
  ...
  function buildShipment(data, options) {
    ...
    data.invoice_line_total = data.invoice_line_total || {};
    ...
    const shipment = {
      ...
      ShipTo: {
        ...
        AttentionName:
          data.ship_to.attention_name || data.ship_to.company_name || '',
      ...
      InvoiceLineTotal: {
        MonetaryValue: data.invoice_line_total.monetary_value || '',
        CurrencyCode: data.invoice_line_total.currency_code || '',
      },
    };

    if (data.shipper.tax_identification_number) {
      shipment.Shipper.TaxIdentificationNumber =
        data.shipper.tax_identification_number;
    }

    if (data.ship_to.location_id) {
      shipment.ShipTo.TaxIdentificationNumber =
        data.ship_to.tax_identification_number;
    }
    ...
    if (data.ship_from) {
      ...
      if (data.ship_from.tax_identification_number) {
        shipment.ShipFrom.TaxIdentificationNumber =
          data.ship_from.tax_identification_number;
      }

      if (
        data.ship_from.tax_id_type &&
        tax_id_types[data.ship_from.tax_id_type]
      ) {
    ...
    if (data.sold_to) {
      ...
      if (data.sold_to.tax_identification_number) {
        shipment.SoldTo.TaxIdentificationNumber =
          data.sold_to.tax_identification_number;
      }
    }
    ...
    if (data.saturday_delivery) {
      if (
        ...
        !(shipment.ShipmentServiceOptions instanceof Array)
      ) {
        shipment.ShipmentServiceOptions = [];
      }
    ...
    if (data.service && default_services[data.service]) {
      shipment.Service = { Code: data.service, };
    }

    if (data.return_service && return_services[data.return_service]) {
      shipment.ReturnService = { Code: data.return_service, };
    }

    for (let i = 0; i < data.packages.length; i++) {
      const p = {
        Package: {
          PackagingType: { Code: data.packages[i].packaging_type || '00', },
          PackageWeight: {
            Weight: data.packages[i].weight || '',
            UnitOfMeasurement: {
              Code: options.use_ounces
                ? 'OZS'
                : $scope.options.imperial
                  ? 'LBS'
                  : 'KGS',
            },
          },
      ...
      if (data.packages[i].reference_number) {
        ...
        p.Package.ReferenceNumber.Value =
          data.packages[i].reference_number;
      }
      ...
      if (data.packages[i].dimensions) {
        p.Package.Dimensions = {
          ...
          UnitOfMeasurement: { Code: $scope.options.imperial ? 'IN' : 'CM', },
        };
      }
      ...
      if (data.packages[i].delivery_confirmation_type) {
        ...
        p.Package.PackageServiceOptions.DeliveryConfirmation = { DCISType: data.packages[i].delivery_confirmation_type || '2', };
      }

      if (
        data.packages[i].reference_number &&
        typeof data.packages[i].reference_number === 'string'
      ) {
        p.Package.ReferenceNumber = { Value: data.packages[i].reference_number, };
      } else if (
        data.packages[i].reference_number &&
        typeof data.packages[i].reference_number === 'object' &&
        !(data.packages[i].reference_number instanceof Array)
      ) {
        // Code/Value key pair
        p.Package.ReferenceNumber = {
          ...
          BarCodeIndicator:
          data.packages[i].reference_number.bar_code_indicator,
        };
      } else if (
        data.packages[i].reference_number &&
        data.packages[i].reference_number instanceof Array
  ...
  function buildPaymentInformation(data, options) {
    data.shipper = data.shipper || { address: {} };
    const payment = { Prepaid: { BillShipper: { AccountNumber: data.shipper.shipper_number || '', }, }, };
  ...
  $scope.dimensionalWeight = function (weight, length, width, height) {
    const dimWeight =
      (length * width * height) /
      ($scope.options.imperial
        ? dimensional_weight_values.imperial
        : dimensional_weight_values.metric);
    if (dimWeight > weight) {
  ...
  $scope.buildRatesRequest = function (data, options) {
    ...
    let request = {
      Request: {
        RequestAction: 'Rate',
        RequestOption:
          data.service && data.service.length > 0 ? 'Rate' : 'Shop',
        TransactionReference: { CustomerContext: options.transaction_id || '', },
      },
      PickupType: { Code: data.pickup_type_code || pickup_codes[data.pickup_type], },
      CustomerClassification: { Code: data.customer_classification || '00', },
    };
  ...
  $scope.handleRatesResponse = function (json, callback) {
    if (json instanceof Array) {
      ...
      for (let i = 0; i < json.length; i++) {
        const j = json[i];
        if (
          j.RatingServiceSelectionResponse.Response.ResponseStatusCode !== '1'
        ) {
          err = j;
          continue;
        }
    ...
    if (
      json.RatingServiceSelectionResponse.Response.ResponseStatusCode !== '1'
    ) {
      return callback(
        json.RatingServiceSelectionResponse.Response.Error,
        null
      );
    }
  ...
  $scope.buildShipmentConfirmRequest = function (data, options) {
    ...
    request.Shipment.Service = {
      ...
      Description:
        default_services[data.service_code] || default_services['03'],
    };
    request.Shipment.PaymentInformation = buildPaymentInformation(
      data,
      options
    );
  ...
  $scope.buildVoidShipmentRequest = function (data, options) {
    ...
    if (typeof data === 'string') {
      ...
      if (data.shipment_identification_number) {
        request.ExpandedVoidShipment.ShipmentIdentificationNumber =
          data.shipment_identification_number;
      }

      if (data.tracking_number) {
        request.ExpandedVoidShipment.TrackingNumber =
      ...
      if (data.tracking_numbers) {
        ...
        for (let i = 0; i < data.tracking_numbers.length; i++) {
          request.ExpandedVoidShipment['#list'].push({ TrackingNumber: data.tracking_numbers[i], });
        }
      }
    }
  ...
  $scope.buildTimeInTransitRequest = function (data, options) {
    ...
    let request = {
      ...
      ShipmentWeight: {
        UnitOfMeasurement: { Code: $scope.options.imperial ? 'LBS' : 'KGS', },
  ...
  $scope.buildPickupRequest = function (data, options, resource, callback) {
    soap.createClient(
      path.join(__dirname, 'wsdl', 'pickup', 'Pickup.wsdl'),
      { endpoint: `https://${hosts[$scope.options.environment]}${resource.p}` },
      function (err, client) {
        if (err) {
          return callback(err, null);
        }
        ...
        client.addSoapHeader(
          `<upss:UPSSecurity xmlns:upss="http://www.ups.com/XMLSchema/XOLTWS/UPSS/v1.0"><upss:UsernameToken><upss:Username>${
            $scope.options.username
          }</upss:Username><upss:Password>${
            $scope.options.password
          }</upss:Password></upss:UsernameToken><upss:ServiceAccessToken><upss:AccessLicenseNumber>${
            $scope.options.access_key
          }</upss:AccessLicenseNumber></upss:ServiceAccessToken></upss:UPSSecurity>`
        );

        const params = {
          Request: {
            RequestOption: data.request_option || 1,
            TransactionReference: { CustomerContext: data.customer_context, },
          },
          ...
          PickupDateInfo: {
            ...
            ReadyTime: data.earliest_time_ready,
        ...
        for (let i = 0; i < data.pickup_piece.length; i++) {
          params.PickupPiece.push({
            ...
            DestinationCountryCode:
              data.pickup_piece[i].destination_country_code || 'US',
      ...
      function (err, client) {
        ...
        const params = {
          ...
          ShipFrom: {
            ...
            Address: {
              AddressLine: [
                data.ship_from.address.address_line_1,
                data.ship_from.address.address_line_2,
                data.ship_from.address.address_line_3,
              ],
          ...
          ShipTo: {
            ...
            Address: {
              AddressLine: [
                data.ship_to.address.address_line_1,
                data.ship_to.address.address_line_2,
                data.ship_to.address.address_line_3,
              ],
  ...
  $scope.buildCancelPickupRequest = function (
    data,
    options,
    resource,
    callback
  ...
  $scope.buildFreightRateRequest = function (
    data,
    options,
    resource,
    callback
    ...
    soap.createClient(
      path.join(__dirname, 'wsdl', 'freight_rate', 'FreightRate.wsdl'),
      { endpoint: `https://${hosts[$scope.options.environment]}${resource.p}` },
      function (err, client) {
        if (err) {
          return callback(err, null);
        ...
        const params = {
          ...
          PaymentInformation: {
            Payer: {
              ...
              Address: {
                AddressLine: [
                  ...
                  data.payer.address.address_line_2,
                  data.payer.address.address_line_3,
                ],
            ...
            ShipmentBillingOption: { Code: data.billing_option, },
          },
          Service: { Code: data.service_code, },
        };

        if (data.handling_unit_one) {
          params.HandlingUnitOne = {
            Quantity: data.handling_unit_one.quantity,
            Type: { Code: data.handling_unit_one.code, },
          };
        }
        ...
        if (data.commodity instanceof Array) {
          ...
          for (let i = 0; i < data.commodity.length; i++) {
            const d = {
              ...
              PackagingType: { Code: data.commodity[i].packaging_type, },
            };
            ...
            if (data.commodity[i].dangerous_goods_indicator) {
              d.DangerousGoodsIndicator =
                data.commodity[i].dangerous_goods_indicator;
            }
          ...
          params.Commodity = {
            ...
            PackagingType: { Code: data.commodity.packaging_type, },
          };
          ...
          if (data.commodity.dangerous_goods_indicator) {
            params.Commodity.DangerousGoodsIndicator =
              data.commodity.dangerous_goods_indicator;
          }
          ...
          if (data.commodity.nmfc_commodity_code) {
            params.Commodity.NMFCCommodityCode =
              data.commodity.nmfc_commodity_code;
          }
  ...
  $scope.buildFreightShipRequest = function (
    data,
    options,
    resource,
    callback
    ...
    if (data.shipment_account_type) {
      params.Shipment.AccountType = { Code: data.shipment_account_type, };
    }
    ...
    if (data.commodity instanceof Array) {
      ...
      if (data.commodity.dangerous_goods_indicator) {
        params.Shipment.Commodity.DangerousGoodsIndicator =
          data.commodity.dangerous_goods_indicator;
      }
      ...
      if (data.commodity.nmfc_commodity_code) {
        params.Shipment.Commodity.NMFCCommodityCode =
          data.commodity.nmfc_commodity_code;
      }
    ...
    */
    ...
    header +=
      `<soap:Header><upss:UPSSecurity xmlns:upss="http://www.ups.com/XMLSchema/XOLTWS/UPSS/v1.0"><upss:UsernameToken><upss:Username>${
        $scope.options.username
      }</upss:Username><upss:Password>${
        $scope.options.password
      }</upss:Password></upss:UsernameToken><upss:ServiceAccessToken><upss:AccessLicenseNumber>${
        $scope.options.access_key
      }</upss:AccessLicenseNumber></upss:ServiceAccessToken></upss:UPSSecurity></soap:Header>`;
    const body_request = builder.create('Request', { headless: true });
    const body_shipment = builder.create('Shipment', { headless: true });

    let body =
      `${header
      }<soap:Body><FreightShipRequest xmlns="http://www.ups.com/XMLSchema/XOLTWS/FreightShip/v1.0">${
        body_request
          .att('xmlns', 'http://www.ups.com/XMLSchema/XOLTWS/Common/v1.0')
          .ele(params.Request)
          .end()}`;
  ...
  $scope.buildFreightPickupRequest = function (
    data,
    options,
    resource,
    callback
    ...
    soap.createClient(
      path.join(__dirname, 'wsdl', 'freight_pickup', 'FreightPickup.wsdl'),
      { endpoint: `https://${hosts[$scope.options.environment]}${resource.p}` },
      function (err, client) {
        if (err) {
          return callback(err, null);
  ...
  $scope.buildCancelFreightPickupRequest = function (
    data,
    options,
    resource,
    callback
  ...
  const resources = {
    rates: {
      p: '/ups.app/xml/Rate',
      f: $scope.buildRatesRequest,
      r: $scope.handleRatesResponse,
    },
    track: {
      p: '/ups.app/xml/Track',
      f: $scope.buildTrackingRequest,
      r: $scope.handleTrackingResponse,
    },
    confirm: {
    ...
    void: {
      p: '/ups.app/xml/Void',
      f: $scope.buildVoidShipmentRequest,
      r: $scope.handleVoidShipmentResponse,
    },
    address_validation: {
      p: '/ups.app/xml/XAV',
    ...
    pickup: {
      ...
      f: $scope.buildPickupRequest,
      r: $scope.handlePickupResponse,
      wsdl: true,
    },
    pickup_rate: {
      p: '/webservices/Pickup',
